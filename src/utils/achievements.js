...
/**
 * Calculates achievement data based on user stats
 * @param {Object} achievements - User achievement stats from API
 * @returns {Array} Array of achievement objects with title, description, progress, and icon
 */
export const getAchievementData = (achievements) => {
...
/**
 * Gets only the titles of completed achievements
 * @param {Object} achievements - User achievement stats from API
 * @returns {Array<string>} Array of achievement titles that are completed (progress >= 100)
 */
export const getCompletedAchievementTitles = (achievements) => {
  const achievementData = getAchievementData(achievements);
  return achievementData
    .filter(achievement => achievement.progress >= 100)
    .map(achievement => achievement.title);
};

/**
 * Gets full achievement objects for completed achievements
 * @param {Object} achievements - User achievement stats from API
 * @returns {Array<Object>} Array of completed achievement objects
 */
export const getCompletedAchievements = (achievements) => {
  const achievementData = getAchievementData(achievements);
  return achievementData.filter(achievement => achievement.progress >= 100);
};
